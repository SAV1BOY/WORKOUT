...
.ato:first-of-type{border:0;background:var(--destaque);color:var(--sobre)}
.ato:last-of-type{border:1px solid var(--linha);background:transparent;color:var(--texto);font-weight:400}`;
...
<h1>${TITULO}</h1>
<p>${FRASE}</p>
<button type="button" class="ato" onclick="location.reload()">${TENTAR}</button>
<a class="ato" href="/">${IR_PARA_O_TREINO}</a>
</main></body></html>`;
}
